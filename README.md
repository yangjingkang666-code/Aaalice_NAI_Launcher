...
- **项目工作区**：可在设置中选择一个项目目录，将 `images/`、图片元数据、Recipe 和项目词库一起管理；旧图库与 Recipe 支持非破坏导入，切换项目会按路径重建图库索引。
...
- **项目工作区**：在“设置 → 数据与存储”点击项目目录，选择或创建项目；需要迁移旧数据时点击导入，已有项目文件不会被覆盖。关闭项目即可回到旧版全局图片目录。
- **标签补全**：基础标签库随应用提供，可离线使用。相关标签推荐、中文标签词库和 AI 翻译属于可选数据源，在“设置 → 数据源与缓存”中管理。
- **本地图片反推**：在“设置 → 数据与存储 → 本地反推模型管理”导入 JoyTag/WD EVA02 的 ONNX 与标签文件（JoyTag 通常为 `joytag.onnx` + `top_tags.txt`；WD EVA02 通常为 `wd-eva02*.onnx` + `selected_tags.csv`）。Windows 默认优先尝试 DirectML，运行库或驱动不满足时自动回退 CPU；模型不会随应用静默下载。
